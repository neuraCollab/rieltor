...
const HousesAndLandsFilters: React.FC<Props> = ({ onFilter }) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/infrastructures/count?${query.toString()}`, { cache: 'no-store' })
      .then((res) => res.json())
  ...
  return (
    <form onSubmit={handleSubmit(() => {})} className="flex flex-wrap gap-4">
      ...
      <label className="flex items-center space-x-2">
        <input type="checkbox" {...register('withLand')} />
        <span>С земельным участком</span>
      </label>
      ...
      <div className="w-full flex justify-between items-center mt-4">
        ...
        <button type="submit" className="px-4 py-2 bg-red-500 text-white rounded">
          Показать {count?.toLocaleString() || 0} предложений
        </button>
      </div>
    </form>
  )
...
export default HousesAndLandsFilters
